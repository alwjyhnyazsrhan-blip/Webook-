...
class DiscoveryEngine:
    ...
    # Webook API internal type labels - never show as user-visible categories
    ...
    async def get_live_categories(self) -> list:
        async with AsyncSessionLocal() as db:
            # Active genres only, excluding internal API type labels
            stmt = (
                select(Genre)
    ...
    async def _sync_all_locked(self):
        ...
        async with AsyncSessionLocal() as db:
            ...
            # 2. SITEMAP EXHAUSTIVE SCAN (High Confidence)
            try:
    ...
    async def _hydrate_event(self, db, event: LiveEvent, client: WebookApiClient) -> bool:
        ...
        try:
            ...
            if not img_url:
                img_url = data.get("mobile_poster") or data.get("poster") or data.get("promo_poster")
    ...
    async def _upsert_event_shell(self, db, ev: dict, genre_id: int = None):
        ...
        if genre_id is None:
            ...
            if isinstance(cat_raw, dict):
                cat_name = str(cat_raw.get("title") or cat_raw.get("name") or cat_raw.get("label") or "")
            elif isinstance(cat_raw, str):
                cat_name = cat_raw
                
            raw_name = cat_name or (ev_type if ev_type not in self._INTERNAL_GENRE_BLACKLIST else None) or genre_slug
            
            if genre_slug and genre_slug not in self._INTERNAL_GENRE_BLACKLIST:
                genre_id = await self._get_or_create_genre(db, genre_slug, str(raw_name))
                self.metrics["categorized"] += 1
            else:
                self.metrics["uncategorized"] += 1
    ...
    async def _get_or_create_genre(self, db, slug: str, name: str) -> int:
        ...
        if slug in self._INTERNAL_GENRE_BLACKLIST:
            logger.warning(f"[GENRE_BLACKLIST] Refused blacklisted slug='{slug}'"
            )
            return None
        if isinstance(name, dict):
            name = str(name.get("title") or name.get("name") or name.get("label") or slug.replace("-", " ").title())
        name = str(name or slug.replace("-", " ").title())
        if name.lower().strip() in self._INTERNAL_GENRE_BLACKLIST:
            name = slug.replace("-", " ").title()

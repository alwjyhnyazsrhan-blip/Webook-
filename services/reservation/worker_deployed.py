...
class ReservationWorker:
    ...
    def _is_checkout_success(res) -> bool:
        ...
        if res.get("order_id") or res.get("payment_session_id") or res.get("payment_url"):
            return True
        data = res.get("data")
        if isinstance(data, dict) and (data.get("order_id") or data.get("payment_session_id") or data.get("payment_url")):
            return True
        return False
        ...
        try:
            payload = {
                ...
                "task": {
                    ...
                    "userAgent": ctx.user_agent, # CRITICAL: Sync identity with solver
                },
            }
    ...
    async def _build_seatcloud_selection(
        ...
        # event_key is the SeatCloud INSTANCE key (e.g. a7a06024-...).
        # chart_key is the TEMPLATE key (e.g. 8e8f26aa-...).
        # All SeatCloud API calls (/event/{id}/data, /items/held, WS ?event=) need event_key.
        event_key = (
        ...
        # Use event_key for API calls; fall back to chart_key if event_key is absent
        sc_api_key = event_key or chart_key
        workspace_key = (
        ...
        if not sc_api_key or not workspace_key or not category_key:
            logger.error(
        ...
        logger.info(
            f"[SEATCLOUD_REQUEST] workspace_key={workspace_key} chart_key={sc_api_key} "
            f"url=https://api.seatcloud.com/api/v2/{workspace_key}/event/{sc_api_key}/data?plain=true"
        )
        chart = await client.get_seatcloud_chart_data(
            workspace_key=workspace_key, chart_key=sc_api_key
        )
        if not chart or not isinstance(chart, dict):
            logger.error(
                f"[SEAT_SELECTION] correlation_id={ctx.correlation_id} task_id={task.id} "
                f"ok=False reason=INVALID_CHART_DATA"
            )
            return []
        ...
        ticket_id = str(ticket.get("_id") or ticket.get("id") or "")

        # Diagnostic: Log available keys in the chart
        chart_keys = set()
        for sec in sections:
            k = sec.get("specification", {}).get("key")
            if k: chart_keys.add(str(k).strip())
        for area in areas:
            k = area.get("specification", {}).get("key")
            if k: chart_keys.add(str(k).strip())
            
        logger.info(f"[SEAT_SELECTION] task_id={task.id} target_key={category_key} chart_keys={list(chart_keys)}")

        target_section_ids = set()
        for sec in sections:
            sec_key = str(sec.get("specification", {}).get("key") or "").strip()
            if sec_key == category_key:
                target_section_ids.add(sec.get("id"))

        category_chairs = [
            c for c in chairs
            if (not target_section_ids or c.get("sectionId") in target_section_ids)
            and str(c.get("specification", {}).get("key") or "").strip() == category_key
        ]
        chairs_with_occupancy = [c for c in category_chairs if isinstance(c.get("occupancy"), dict)]
        available_chairs = [
            c for c in chairs_with_occupancy
            if str(c.get("occupancy", {}).get("status") or "").lower() == "available"
        ]

        if category_chairs and not chairs_with_occupancy:
            # seatcloud /data?plain=true returns static layout only — no real-time occupancy.
            # Shuffle and exclude already-tried seats. Webook validates at checkout.
            import random as _random
            filtered = [c for c in category_chairs if c.get("id") not in (exclude_seat_ids or set())]
            _random.shuffle(filtered)
            logger.warning(
                f"[SEAT_SELECTION] correlation_id={ctx.correlation_id} task_id={task.id} "
                f"ok=True reason=NO_OCCUPANCY_FALLBACK category_key={category_key} "
                f"chairs={len(category_chairs)} available_after_exclude={len(filtered)}"
            )
            available_chairs = filtered

        if len(available_chairs) >= int(task.seat_count or 1):
            # Exclude already-tried seats for retries
            pool = [c for c in available_chairs if c.get("id") not in (exclude_seat_ids or set())]
            selected = []
            for chair in pool[:task.seat_count]:
                spec = chair.get("specification", {})
                selected.append({
                    "id": chair.get("id"),
                    "sectionId": chair.get("sectionId"),
                    "rowName": chair.get("rowName"),
                    "label": chair.get("label", {}).get("label") if isinstance(chair.get("label"), dict) else chair.get("label"),
                    "itemType": "seat",
                    "objectType": "seat",
                    "category": {
                        "key": str(category_key),
                        "label": spec.get("label") or "Selected"
                    },
                    "categoryKey": str(category_key),
                    "numSelected": 1,
                    "selectedTicketType": str(ticket_id),
                    "selectionPerTicketType": {str(ticket_id): 1},
                })
            return selected

        for area in areas:
            spec = area.get("specification", {})
            if str(spec.get("key")) == category_key:
                return [{
                    "id": area.get("id"),
                    "name": area.get("name"),
                    "label": area.get("label", {}).get("label") if isinstance(area.get("label"), dict) else area.get("label"),
        ...
        logger.error(
            f"[SEAT_SELECTION] correlation_id={ctx.correlation_id} task_id={task.id} "
            f"ok=False reason=CATEGORY_NOT_FOUND category_key={category_key}"
        )
        return []
    ...
    async def _record_checkout_result(
        ...
        if isinstance(res, dict) and str(res.get("status", "")).lower() == "success":
            ...
            order_id           = data.get("order_id") or data.get("reservationId") or data.get("reservation_id")
            ...
            payment_link = ""
            if payment_url:
                payment_link = f'<a href="{payment_url}">\U0001f517 \u0625\u062a\u0645\u0627\u0645 \u0627\u0644\u062f\u0641\u0639 \u0627\u0644\u0622\u0646</a>'

            await self._notify_user(
                task.user_id,
                f"\U0001f389 <b>\u062a\u0645 \u0627\u0644\u062d\u062c\u0632 \u0628\u0646\u062c\u0627\u062d!</b>\n"
                f"\U0001f3ad \u0627\u0644\u0641\u0639\u0627\u0644\u064a\u0629: <code>{task.event_slug}</code>\n"
                f"\U0001f522 \u0639\u062f\u062f \u0627\u0644\u062a\u0630\u0627\u0643\u0631: {task.seat_count}\n"
                f"\U0001f516 \u0631\u0642\u0645 \u0627\u0644\u0637\u0644\u0628: <code>{order_id or payment_session_id or '--'}</code>\n"
                f"{payment_link}",
                task_id=task.id,
            )
            return
        ...
        async with AsyncSessionLocal() as db:
            ...
            try:
                ...
                async def _ctx_request(method, url, authenticated=False, **kwargs):
                    ...
                    if authenticated and account.bearer_token:
                ...
                if not is_seated and not is_best_available:
                    res = await client.checkout_plain(task.event_slug, event_id, ticket_order, time_slot_id=task.timeslot_id)
                ...
                hold_data = await client.hold_token(task.event_slug, event_id, time_slot_id=task.timeslot_id)
                ...
                captcha_token = None
                if isinstance(hold_data, dict) and hold_data.get("_http_status") == 422:
                    captcha_token = await self._solve_turnstile_captcha(task.event_slug, ctx)
                    if captcha_token:
                        hold_data = await client.hold_token(task.event_slug, event_id, time_slot_id=task.timeslot_id, captcha_token=captcha_token)
                        if isinstance(hold_data, dict):
                            hold_token_value = hold_data.get("hold_token") or hold_data.get("holdToken") or hold_data.get("token")
                ...
                # PROFILE ELIGIBILITY CHECK once before retry loop
                try:
                ...
                # SEATED RETRY LOOP
                # Always use /event-seat/checkout (the browser endpoint).
                # NEVER fall back to /checkout/hold-best-available — that endpoint
                # returns 422 "not authorized" for RSL/SPL events unconditionally.
                # On "invalid ticket" errors (seat already taken from static map),
                # retry with a fresh shuffled seat selection, up to MAX_SEAT_RETRIES.
                MAX_SEAT_RETRIES = 5
                ...
                for attempt in range(1, MAX_SEAT_RETRIES + 1):
                    ...
                    for seat in seats_this_attempt:
                        seat["selectedTicketType"] = str(ticket_id)
                        seat["selectionPerTicketType"] = {str(ticket_id): 1}

                    tried_seat_ids.update(s.get("id") for s in seats_this_attempt if s.get("id"))
                    seat_labels = [s.get("label") or s.get("name") or s.get("id") for s in seats_this_attempt]

                    logger.info(
                        f"[SEATED_RETRY_ATTEMPT] correlation_id={ctx.correlation_id} "
                        f"task_id={task.id} attempt={attempt}/{MAX_SEAT_RETRIES} "
                        f"seat_ids={[s.get('id') for s in seats_this_attempt]}"
                    )

                    # SEAT REGISTRATION via SeatCloud WebSocket (mirrors browser seat-click).
                    # The browser JS widget connects wss://api.seatcloud.com:8443/?event={event_key}
                    # and sends a HOLD message. The server registers the seat to the holdToken.
                    # We then call GET /items/held to get the allocation UUID per seat.
                    # Webook validates this UUID at /event-seat/checkout → "Invalid Tickets"
                    # if the seat is not registered to the token.
                    if hold_token_value:
                        seats_io_meta = detail.get("seats_io") or detail.get("seats") or {}
                        sc_workspace = (
                            (seats_io_meta.get("workspace_key") or seats_io_meta.get("workspaceKey"))
                            if isinstance(seats_io_meta, dict) else None
                        )
                        # CRITICAL: use event_key (instance), NOT chart_key (template)
                        sc_event_key = (
                            (
                                seats_io_meta.get("event_key") or seats_io_meta.get("eventKey")
                                or seats_io_meta.get("event_id") or seats_io_meta.get("eventId")
                                or detail.get("event_key") or detail.get("eventKey")
                                # fallback to chart_key only if no event_key at all
                                or seats_io_meta.get("chart_key") or seats_io_meta.get("chartKey")
                                or seats_io_meta.get("chart_token") or seats_io_meta.get("chartToken")
                                or detail.get("chart_key") or detail.get("chartKey")
                            )
                            if isinstance(seats_io_meta, dict) else None
                        )
                        if sc_workspace and sc_event_key:
                            # Hold each seat via WebSocket using its label (section-row-seat format)
                            for seat in seats_this_attempt:
                                seat_label = (
                                    seat.get("label") or seat.get("name") or seat.get("id")
                                )
                                if seat_label:
                                    await client.hold_seat_via_websocket(
                                        sc_workspace, sc_event_key, hold_token_value, str(seat_label)
                                    )

                            # Poll items/held to get allocation UUIDs assigned by SeatCloud
                            import asyncio as _asyncio
                            await _asyncio.sleep(0.5)  # brief wait for server to process hold
                            held_items = await client.get_held_items_seatcloud(
                                sc_workspace, sc_event_key, hold_token_value
                            )
                            # Build label→uuid map (uuid is the allocation Webook validates)
                            alloc_map = {
                                item.get("label"): item.get("uuid")
                                for item in held_items
                                if isinstance(item, dict) and item.get("label") and item.get("uuid")
                            }
                            for seat in seats_this_attempt:
                                seat_label = str(seat.get("label") or seat.get("name") or seat.get("id") or "")
                                if seat_label and seat_label in alloc_map and alloc_map[seat_label]:
                                    seat["allocation"] = alloc_map[seat_label]

                            logger.info(
                                f"[SEATCLOUD_HOLD_DONE] correlation_id={ctx.correlation_id} "
                                f"task_id={task.id} attempt={attempt} "
                                f"held_count={len(held_items)} alloc_map={alloc_map}"
                            )
                        else:
                            logger.warning(
                                f"[SEATCLOUD_HOLD_SKIP] correlation_id={ctx.correlation_id} "
                                f"task_id={task.id} sc_workspace={bool(sc_workspace)} sc_event_key={bool(sc_event_key)}"
                            )
                    ...
                    if self._is_checkout_success(res):
                        ...
                        await self._record_checkout_result(
                            ...
                            ctx=ctx, hold_token=hold_token_value, seat_labels=seat_labels,
                        )
                        return
                ...
                if account:
                    try:
                        # Save cookies for next session with timestamp to avoid poisoning
                        from core.network.session import network_manager
                        current_cookies = network_manager.get_cookies(str(account.id))
                        if current_cookies:
                            cookie_data = {
                                "ts": datetime.now(timezone.utc).isoformat(),
                        ...
                        logger.warning(f"[FINALLY_CLEANUP_FAIL] account_id={account.id} error={e}")


    async def _notify_user(self, user_id: int, text: str, task_id: int = None):
        ...
        try:
            ...
            if task_id:
                builder.row(tg_types.InlineKeyboardButton(text="\U0001f50d \u062a\u0641\u0627\u0635\u064a\u0644 \u0627\u0644\u0639\u0645\u0644\u064a\u0629", callback_data=f"task_detail:{task_id}"))
            builder.row(tg_types.InlineKeyboardButton(text="\U0001f4cb \u0627\u0644\u0639\u0645\u0644\u064a\u0629", callback_data="list_tasks"))
    ...
    async def _fail_task(self, db, task, reason: str, ctx: Optional[ReservationSessionContext] = None):
        ...
        task.execution_logs = logs
        await db.commit()
        await self._notify_user(task.user_id, f"\u274c <b>\u0641\u0634\u0644 \u0627\u0644\u062d\u062c\u0632</b>\n\U0001f3ad \u0627\u0644\u0641\u0639\u0627\u0644\u064a\u0629: <code>{task.event_slug}</code>\n\U0001f4cc \u0627\u0644\u0633\u0628\u0628: <code>{reason[:200]}</code>", task_id=task.id)
